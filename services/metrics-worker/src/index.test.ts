...
const connection: AwsConnection = {
  ...
  tenantId: agent('agt_00000000-0000-4000-8000-000000000001').tenantId,
  accountId: '123456789012',
  region: 'us-east-1',
  roleArn: 'arn:aws:iam::123456789012:role/AgentLaunchpadDeploymentRole',
  externalId: 'secret-not-persisted',
  status: 'VERIFIED',
  createdBy: 'system',
  createdAt: now.toISOString(),
  updatedAt: now.toISOString()
...
test('collects normalized metrics and isolates an agent failure', async () => {
  ...
  const agents = [
    agent('agt_00000000-0000-4000-8000-000000000001'),
    agent('agt_00000000-0000-4000-8000-000000000002')
  ];
  const repository = {
    listActiveDeployedAgents: async () => ({ items: agents, nextToken: undefined }),
    getAwsConnection: async () => connection,
    putAgentMetricsSnapshot: async (value: unknown) => {
      writes.push(value);
    },
    getAgent: async () => agent('agt_00000000-0000-4000-8000-000000000001')
  };
  const reader: AgentCoreMetricsReader = {
    read: async ({ agent: item }) => {
...
test('revoked access does not overwrite the most recent successful snapshot', async () => {
  let writes = 0;
  const repository = {
    getAwsConnection: async () => ({ ...connection, status: 'DISCONNECTED' }),
    putAgentMetricsSnapshot: async () => {
      writes += 1;
    }
  };
  const worker = new MetricsWorker({
  ...
  await assert.rejects(
    worker.collectAgent(agent('agt_00000000-0000-4000-8000-000000000001')),
